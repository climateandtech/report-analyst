...
class FlexibleDatasetRow(BaseModel):
    ...
    """
    data: Dict[str, Any] = Field(..., description="Flexible data dictionary containing all row fields")
    ...
    def get_score(self) -> Optional[float]:
        ...
        score = self.get('score') or self.get('relevance_score') or self.get('confidence_score') or self.get('similarity_score')
    ...
    def get_answer(self) -> Optional[str]:
        ...
        return self.get('answer') or self.get('analysis') or self.get('text') or self.get('response')
    
    def get_category(self) -> Optional[str]:
        """Get category/classification"""
        return self.get('category') or self.get('class') or self.get('label') or self.get('type')
...
class BenchmarkDataset(BaseModel):
    ...
    question_set: Optional[str] = Field(None, description="Question set this dataset is for")
    ...
    dataset_type: Optional[DatasetType] = Field(None, description="Type of dataset: IR or IE (None for metadata-only)")
    source: Optional[str] = Field(None, description="Source: 'csv', 'sqlite', 'internal', 'database'")
    source_path: Optional[str] = Field(None, description="Path to source file/database")
    
    # Data and mapping (for loaded datasets)
    column_mapping: Dict[str, str] = Field(default_factory=dict, description="Mapping of standard names to actual column names")
    results: List[FlexibleDatasetRow] = Field(default_factory=list, description="List of dataset rows (empty for metadata-only)")
    ...
    def get_unique_reports(self) -> List[str]:
        ...
        report_ids = [r.get('report_id') or r.get('document_id') or r.get('doc_id') 
...
class RetrievalResultRow(BaseModel):
    ...
    position: int = Field(..., ge=1, description="Position/rank in retrieval results (1-indexed)")
    score: float = Field(..., description="Retrieval score (similarity, relevance, etc.)")
...
class RetrievalResultsDataset(BaseModel):
    ...
    source_path: Optional[str] = Field(None, description="Path to source file/database")
    results: List[RetrievalResultRow] = Field(..., description="List of retrieval results")
    created_at: Optional[datetime] = Field(None, description="Creation timestamp")
