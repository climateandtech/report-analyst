...
class BenchmarkStore:
    ...
    def save_dataset(self, dataset: BenchmarkDatasetContent, file_path: str) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("DELETE FROM ground_truth_chunks WHERE dataset_id = ?", (dataset.dataset_id,))
    ...
    def list_evaluations(self, dataset_id: Optional[str] = None) -> List[BenchmarkEvaluation]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in cursor.fetchall():
                ...
                evaluations.append(BenchmarkEvaluation(
                    id=row['id'],
                    dataset_id=row['dataset_id'],
                    evaluation_name=row['evaluation_name'],
                    config_hash=row['config_hash'],
    ...
    def delete_dataset(self, dataset_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("DELETE FROM human_annotations WHERE evaluation_id IN (SELECT id FROM benchmark_evaluations WHERE dataset_id = ?)", (dataset_id,))
