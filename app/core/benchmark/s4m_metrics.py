...
def evaluate_s4m_classification(
    ...
    # Process model predictions: round to nearest integer, then clamp to [0, 2]
    ...
    # Sort by model prediction score (descending) within each group
    df = df.sort_values(
        by=group_by + [model_pred_col],
    ...
    # Evaluate each group (criteria-company pair)
    for group_key, g in df.groupby(group_by, sort=False):
        rels_ranked = g[ground_truth_col].tolist()
        bin_ranked = [1 if r >= relevance_threshold else 0 for r in rels_ranked]
        
        gt_rel = int(total_relevant.get(group_key, 0))
