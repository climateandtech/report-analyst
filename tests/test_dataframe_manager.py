import pytest
import pandas as pd
from app.core.dataframe_manager import create_analysis_dataframes, format_list_field
...
def test_create_analysis_dataframes_with_chunks():
    ...
    cached_results = {
        'ev_24': {
            'result': {'ANSWER': 'Test', 'SCORE': 5, 'EVIDENCE': [], 'GAPS': [], 'SOURCES': []},
            'chunks': [
                {
    ...
    assert chunks_df.iloc[0]['Is Evidence'] == True
    assert chunks_df.iloc[1]['Is Evidence'] == False
